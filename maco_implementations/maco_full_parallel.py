...
def macoUpdate(X, X_u, X_l,x_line,x_u_line,x_l_line, row, dimension=15, m=0.0001,epoch_n=50):
    t0 = time()
   ...
   #      sa.delete("shm://"+ str(i[0].decode('UTF-8')))
    ...
    L= sa.attach("shm://L")
    R= sa.attach("shm://R")
    X_u =sa.attach("shm://X_u")
    X_l =sa.attach("shm://X_l")
    X = sa.attach("shm://X")
    ...
    iter = 0
    print("Reconstruction Error Before re-train: ", current)
...
# X_u=np.ones(X.shape)
# L,R=maco(X,X_u,X_l,10,m=0.0001,epoch_n=1000)
#
# res= L.dot(R)
# print(res)